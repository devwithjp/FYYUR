...
def venues():
  ...
  for venue in venues:
    ...
    destinations.add((venue.city, venue.state))

  # for each unique city/state, add venues
  for destination in destinations:
    dest_venues = Venue.query.filter_by(state=destination[1]).filter_by(city=destination[0]).all()
    venue_data = []
    for venue in dest_venues:
      venue_data.append({
        "id": venue.id,
        "name": venue.name, 
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error: 
    flash(f'Error occurred. Venue {venue_id} could not be deleted.')
  if not error: 
...
def edit_venue_submission(venue_id):
  ...
  try: 
    ...
    db.session.close()
  if error: 
    flash('Error occurred. Venue could not be changed.')
  if not error: 
...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  shows_query = Show.query.order_by(db.desc(Show.start_time))
